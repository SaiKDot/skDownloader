...
export default class Api {
  ...
  savePreferenceToNativeStore (params = {}) {
    ...
    if (!isEmpty(user)) {
      console.info('>> App << save user config: ', user)
      config.user = user
    }

    if (!isEmpty(system)) {
      console.info('>> App << save system config: ', system)
      config.system = system
      this.updateActiveTaskOption(system)
    }

    if (!isEmpty(others)) {
      console.info('>> App << save config found illegal key: ', others)
    }
  ...
  async fetchDownloadingTaskList (params = {}) {
    this.config = await this.loadConfig()
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        // console.log('>> App << fetch downloading task list data:', data)
        const result = mergeTaskResult(data)
        resolve(result)
      }).catch((err) => {
        console.log('>> App << fetch downloading task list fail:', err)
        reject(err)
      })
    })
  }
  ...
  fetchTaskItemWithPeers (params = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        console.log('>> App << fetchTaskItemWithPeers:', data)
        const result = data[0] && data[0][0]
        const peers = data[1] && data[1][0]
        result.peers = peers || []
        console.log('>> App << fetchTaskItemWithPeers.result:', result)
        console.log('>> App << fetchTaskItemWithPeers.peers:', peers)
