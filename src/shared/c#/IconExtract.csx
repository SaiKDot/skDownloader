...
public class Startup 
{
    public async Task<object> Invoke(IDictionary<string, object> input)
    {
       ...
       string mr =  (string) input["types"];
       string[] typesarray = mr.Split(",".ToCharArray());      
       Dictionary<string, string> elementsDict =  new Dictionary<string, string>(10); 
        for (int key = 0; key < typesarray.Length; ++key)
        {         
            elementsDict.Add((string)typesarray[key],IconExtractor.Getter(typesarray[key]) );
            
        }

        var kvs = elementsDict.Select(kvp => string.Format("\"{0}\":\"{1}\"", kvp.Key, string.Join(",", kvp.Value)));
        string jsonString =  string.Concat("{", string.Join(",", kvs), "}");
        
        var result = new { 
            imageString = jsonString
         };
...
{ 
    static class IconExtractor
    {
        ...
        public static string Getter(string eextension)
        {
            
             var ext = eextension;
             string iconImage;
             IntPtr hIcon = GetXLIcon(GetIconIndex("*." + ext));
             ...
             using (Icon ico = (Icon)Icon.FromHandle(hIcon).Clone())
             {
               ...
               byte[] data = (byte[])vert.ConvertTo(ico.ToBitmap(), typeof(byte[]));
               
               iconImage = Convert.ToBase64String(data);
